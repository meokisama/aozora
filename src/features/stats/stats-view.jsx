...
export function StatsView() {
  ...
  const [data, setData] = useState(null); // { overview, daily, hourly, perBook }
                ...
                <Card size="sm" className="gap-3">
                  ...
                  <div className="flex items-center justify-between px-3">
                    <p className="text-[11px] text-muted-foreground">
                      {selected
                        ? `${selectedDay} — ${formatCompact(selected.chars)} chars · ${formatDuration(selected.ms)} · ${selected.sessions} session${selected.sessions === 1 ? "" : "s"}`
