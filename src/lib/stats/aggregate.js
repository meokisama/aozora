 ...
 */
export function computeStreaks(activeDays, todayKey) {
  ...
  let cursor = set.has(todayKey) ? todayKey : set.has(shiftDay(todayKey, -1)) ? shiftDay(todayKey, -1) : null;
  let current = 0;
 ...
 */
export function buildHeatmapWeeks(year, valueByDay) {
 ...
 */
export function tierStatus(value, thresholds) {
