...
/**
 * @param {{ pageSpread: string|null, prePaginated?: boolean, linear?: boolean }[]} pages  pages in reading order
 * @param {"ltr"|"rtl"} ppd  page-progression direction
 * @returns {{ index: number, items: object[], single: boolean, pageSpread: string|null }[]}
 */
export function buildSpreads(pages, ppd) {
  const before = ppd === "rtl" ? "right" : "left"; // opens a pair
  const after = ppd === "rtl" ? "left" : "right"; // closes a pair
  const fixed = (p) => p && p.prePaginated !== false; // default true (wholly-fixed manga)
