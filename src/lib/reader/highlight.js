...
import { collectBlocks, normalize } from "@/lib/reader/search";
...
function rangeForBlock(block, start, len) {
...
/**
 * Highlights the search hit at `charOffset` within `rootEl`.
 *
 * @param {Element} rootEl     the rendered content root (.aozora-content or
 *                             .aoz-page-content)
 * @param {number}  charOffset the global character offset of the hit
 * @param {string}  query      the active query
 * @param {number}  [baseChar] global offset of the rendered region's start
 * @returns {boolean} whether a highlight was set
 */
export function highlightSearchResult(rootEl, charOffset, query, baseChar = 0) {
