...
export class PaginatedController {
  /**
   * @param {object}  opts
   * @param {HTMLElement} opts.scrollEl   the overflow-hidden viewport element
   * @param {HTMLElement} opts.contentEl  the multi-column container inside it
   * @param {Element[]}   opts.sections   detached `aoz-<idref>` section elements
   * @param {boolean}     opts.vertical   true for tategaki (vertical-rl)
   * @param {(state: { char: number, page: number, totalPages: number, sectionIndex: number }) => void} [opts.onChange]
   */
  constructor({ scrollEl, contentEl, sections, vertical, onChange }) {
  ...
  _measure() {
    ...
    const pageFirstChar = new Array(this.totalPages);
   ...
   * @param {number} index
   * @param {"start" | "end" | { char: number }} landing
   */
  async setSection(index, landing) {
