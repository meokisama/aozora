...
export function getRenditionProperty(contents, property) {
...
export function getMetaContentByName(contents, name) {
  const metaKey = getMetaKey(contents);
  for (const m of asArray(getMetadata(contents)?.[metaKey])) {
 ...
 */
export function getRenditionLayout(contents) {
...
export function isFixedLayout(contents) {
 ...
 */
export function getBookViewport(contents) {
  return (
...
export function parsePageSpread(properties) {
...
export function parseItemLayout(properties) {
  if (!properties) return null;
  for (const token of String(properties).trim().split(/\s+/)) {
 ...
 */
export function getSpinePageSpreads(contents) {
