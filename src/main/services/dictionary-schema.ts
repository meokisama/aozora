...
/**
 * Pragmas + schema for the dictionary database, applied to any connection.
 * Shared by the main-process connection (dictionary-db.ts) and the import
 * utility process, which opens its own connection to the same WAL file. Kept
 * free of Electron imports so it loads in the utility process. The DDL is
 * idempotent (CREATE … IF NOT EXISTS), so re-applying on a second connection is
 * a no-op.
 */
export function applyDictionarySchema(database: Database.Database): void {
  ...
  database.exec(`
    CREATE TABLE IF NOT EXISTS dictionaries (
      ...
      source_id   TEXT,                        -- recommended-catalog id if installed via one-click (see migrations/migrate-2), else NULL
